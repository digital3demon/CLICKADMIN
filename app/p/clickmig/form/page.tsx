...
export default function ClickMigPublicFormPage() {
  ...
  if (submitted) {
    return (
      <main className="mx-auto max-w-lg p-6">
        ...
        <p className="mt-2">Номер: {submitted}</p>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-2xl space-y-6 p-6">
      <h1 className="text-2xl font-semibold">КликМиг — заказ</h1>
      <form className="space-y-4" onSubmit={(e) => void submit(e)}>
        ...
        <div>
          <label className="text-sm font-medium">Сканы (STL/PLY/OBJ/ZIP)</label>
          <input
            type="file"
            multiple
            className="mt-1 block w-full text-sm"
        ...
        {scanFiles.length > 0 && (
          <ClickMigScanViewer meshUrls={previewUrls.filter((_, i) => /\.(stl|ply|obj)$/i.test(scanFiles[i]?.name ?? ""))} />
        )}
        ...
        <button
          type="submit"
          disabled={busy}
          className="w-full rounded-lg bg-blue-600 py-2.5 text-white disabled:opacity-50"
        >
