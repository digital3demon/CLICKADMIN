...
import { stickerPublicClientPath } from "@/lib/sticker-public-path";
...
export default async function StickerPublicHubPage({
  ...
  const clientPath = stickerPublicClientPath(tenantSlug, token);
  ...
  return (
    <div className="min-h-screen bg-zinc-100 px-4 py-10 text-zinc-900">
      <div className="mx-auto max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
        <h1 className="text-lg font-semibold text-zinc-900">Наряд {order.orderNumber}</h1>
        <p className="mt-2 text-sm text-zinc-600">
          Выберите роль. Для сотрудников лаборатории нужна активная сессия в CRM в этом браузере
          или вход по ссылке.
        </p>
        <ul className="mt-6 space-y-3">
          ...
          <li>
            <Link
              className="block rounded-lg border border-emerald-300 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-950 hover:bg-emerald-100"
              href={clientPath}
            >
              Клиент — статус без входа
            </Link>
            <p className="mt-1 text-xs text-zinc-500">
              Даты поступления, оформления, этапы и участники по данным CRM (без редактирования).
            </p>
          </li>
        </ul>
      </div>
