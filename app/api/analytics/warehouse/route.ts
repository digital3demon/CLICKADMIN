...
import { loadWarehouseReport } from "@/lib/analytics/reports.server";
...
export async function GET(req: Request) {
  try {
    ...
    const data = await loadWarehouseReport(range.from, range.to);
    return NextResponse.json(data);
  } catch (e) {
    console.error("[analytics/warehouse]", e);
