...
export function FinanceOfficeModePanel({
  ...
  const dateInp =
    "h-8 min-w-0 w-full max-w-[9.5rem] flex-1 rounded-md border border-[var(--input-border)] bg-[var(--card-bg)] px-1 py-0.5 text-[11px] text-[var(--app-text)] shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 crm-t2:h-9 crm-t2:w-[7.5rem] crm-t2:max-w-none crm-t2:flex-none crm-t2:text-xs";
  ...
  return (
    <div className="no-print flex h-auto min-h-[3.25rem] min-w-0 items-start rounded-lg border border-[var(--card-border)] bg-[var(--card-bg)] px-2.5 py-1.5 shadow-sm ring-1 ring-black/[0.04] dark:ring-white/[0.06] sm:px-3">
      <div className="flex min-w-0 w-full flex-wrap items-center gap-x-1.5 gap-y-1.5">
        ...
        <form
          ...
          method="get"
          className="flex min-w-0 w-full flex-wrap items-center gap-x-1.5 gap-y-1.5 crm-t2:w-auto"
        >
        ...
        {listSummaryLine ? (
          <p className="min-w-0 w-full basis-full text-[11px] font-medium leading-snug text-[var(--text-body)] crm-t2:text-xs crm-t3:text-sm">
            {listSummaryLine}
          </p>
        ) : null}
      </div>
