...
export function TelegramMiniAppBootstrap() {
  ...
  const run = useCallback(async () => {
    ...
    setFallbackHref(browserFallbackHref(target));

    const initData = wa.initData?.trim() || "";
    if (!initData) {
      setPhase("error");
      setMessage(
        "Откройте ссылку внутри Telegram (Mini App). В обычном браузере вход по initData недоступен.",
      );
      return;
    }
    ...
    if (target?.kind === "order") qs.set("orderId", target.orderId);
      ...
      setPhase("error");
      setMessage("В ссылке нет наряда или карточки.");
      return;
    }

    const cardRes = await fetch(`/api/tg-app/card?${qs.toString()}`, {
      credentials: "same-origin",
    });
    ...
    if (!cardRes.ok) {
      setPhase("error");
      setMessage(cardJson.error || "Карточка не найдена");
      return;
    }
