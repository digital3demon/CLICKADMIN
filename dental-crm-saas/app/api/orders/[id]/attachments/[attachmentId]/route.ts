...
export async function GET(_req: Request, ctx: Ctx) {
  try {
    ...
    const row = await (await getPrisma()).orderAttachment.findFirst({
...
export async function DELETE(_req: Request, ctx: Ctx) {
  try {
    ...
    await (await getPrisma()).order.updateMany({
    ...
    await (await getPrisma()).orderAttachment.delete({ where: { id: attachmentId } });
