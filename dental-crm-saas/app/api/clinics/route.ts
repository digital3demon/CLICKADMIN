...
export async function GET() {
  const prisma = await getPrisma();
  try {
    let rows = await prisma.clinic.findMany({
    ...
    if (stale.length > 0) {
      await Promise.all(
        stale.map((c) =>
          repairDoctorLinksFromOrders(prisma, c.id).catch((e) => {
            console.error("[GET /api/clinics] repair doctor links", c.id, e);
          }),
        ),
      );
    ...
    const [privatePracticeDoctors, allDoctors] = await Promise.all([
