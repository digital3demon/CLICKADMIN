...
- `prisma migrate dev` — создаёт файл SQLite `prisma/dev.db` и применяет миграции.
...
Путь относительный к каталогу `prisma/`, файл базы: `prisma/dev.db`.
...
- Для своего сервера позже заменить SQLite на **PostgreSQL**: в `schema.prisma` сменить `provider` и в `.env` указать строку подключения Postgres.
