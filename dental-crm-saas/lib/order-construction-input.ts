...
export async function buildConstructionCreatesFromInput(
  ...
  | { ok: true; creates: Prisma.OrderConstructionCreateWithoutOrderInput[] }
...
> {
  ...
  const creates: Prisma.OrderConstructionCreateWithoutOrderInput[] = [];
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (plid) {
      let unitPrice = price;
      if (unitPrice == null) {
        const rub = defaultPriceByPlId.get(plid);
    ...
    if ("arch" in c && c.arch != null) {
      ...
      creates.push({
        ...
        constructionType: { connect: { id: t.id } },
        arch: a as JawArch,
        quantity: qty,
        unitPrice: price,
        shade,
