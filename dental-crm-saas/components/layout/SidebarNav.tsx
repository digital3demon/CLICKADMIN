...
const baseNavItems = [
  ...
  { href: "/orders/history", label: "История изменений" },
  { href: "/analytics", label: "Аналитика" },
  { href: "/shipments", label: "Отгрузки" },
  { href: "/warehouse", label: "Склад" },
  { href: "/clients", label: "Клиенты" },
  { href: "/directory", label: "Конфигурация" },
] as const;
...
export function SidebarNav() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const j = (await res.json()) as { user?: { role?: UserRole } | null };
        if (!cancelled) setRole(j.user?.role ?? null);
      } catch {
  ...
  const navItems = useMemo(() => {
    if (role != null && isKanbanOnlyUser(role)) {
      return baseNavItems.filter((i) => i.href === "/kanban");
    }
    const showAnalytics =
      role != null && canAccessFinancialAnalytics(role);
    if (showAnalytics) return [...baseNavItems];
