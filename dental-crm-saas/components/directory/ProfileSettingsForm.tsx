...
> {
  ...
  if (!trimmed) {
    return { ok: false, message: `Пустой ответ сервера (HTTP ${r.status})` };
  }
  try {
          ...
          {!telegramLinked ? (
            <div className="rounded-md border border-[var(--card-border)] bg-[var(--card-bg)] p-3">
              ...
              <ol className="mt-2 list-decimal space-y-1.5 pl-4 text-xs text-[var(--text-secondary)]">
                ...
                <li>
                  Перейдите по ссылке из ответа бота в браузере; после этого привязка активна.
                </li>
              </ol>
              {telegramBotUsername.trim() ? (
                <p className="mt-3">
                  <a
                    href={`https://t.me/${encodeURIComponent(telegramBotUsername.replace(/^@+/, "").trim())}?start=${encodeURIComponent(DEFAULT_TENANT_SLUG)}`}
                    target="_blank"
              ...
              <p className="mt-2 text-[0.65rem] leading-snug text-[var(--text-muted)]">
                На сервере нужны <span className="font-mono">TELEGRAM_BOT_TOKEN</span>,{" "}
                <span className="font-mono">CRM_PUBLIC_BASE_URL</span> и webhook{" "}
                <span className="font-mono">/api/telegram/webhook</span>. Проверка:{" "}
                <a
