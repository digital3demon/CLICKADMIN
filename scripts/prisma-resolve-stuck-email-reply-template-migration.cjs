/**
 * Перед `migrate deploy`: снимает P3009 для миграции 20260529140000_email_reply_template.
 * Типичная причина — SQLite-версия SQL (PRAGMA/DATETIME) на PostgreSQL.
 *
...
const MIGRATION = "20260529140000_email_reply_template";
...
(async () => {
  if (String(process.env.PRISMA_SKIP_STUCK_MIGRATION_FIX || "").trim() === "1") {
    console.log(
      "[prisma-resolve-stuck-email-reply] PRISMA_SKIP_STUCK_MIGRATION_FIX=1 — пропуск.",
    );
    process.exit(0);
  }
  ...
  try {
    ...
    console.warn(
      "[prisma-resolve-stuck-email-reply] нет @prisma/client — пропуск:",
      ...
      `SELECT
        ...
        EXISTS (
          ...
          WHERE t.table_schema = current_schema()
            AND t.table_name = 'EmailReplyTemplate'
        )
        AND EXISTS (
          SELECT 1 FROM information_schema.columns c
          WHERE c.table_schema = current_schema()
            AND c.table_name = 'EmailSourceOrder'
            AND c.column_name = 'isReplyTarget'
        ) AS schema_applied`,
    );
    const row = Array.isArray(rows) ? rows[0] : null;
    if (row && typeof row === "object") {
      stuck = asBool(row.stuck);
      schemaApplied = asBool(row.schema_applied);
    }
  } catch (e) {
    console.warn(
      "[prisma-resolve-stuck-email-reply] запрос к БД не выполнен — пропуск:",
      e instanceof Error ? e.message : e,
    );
  ...
  console.warn(
    `[prisma-resolve-stuck-email-reply] незавершённая миграция ${MIGRATION} (schemaApplied=${schemaApplied}) — migrate resolve.`,
  );
  const flag = schemaApplied ? "--applied" : "--rolled-back";
  const r = runMigrateResolve(flag, MIGRATION);
  const code = r.status === null ? 1 : r.status;
  if (code !== 0) {
    console.error(
      `[prisma-resolve-stuck-email-reply] migrate resolve ${flag} завершился с кодом ${code}`,
    );
    process.exit(code);
  }
  console.warn(`[prisma-resolve-stuck-email-reply] migrate resolve ${flag}: OK`);
  process.exit(0);
})().catch((e) => {
  console.error("[prisma-resolve-stuck-email-reply]", e);
  process.exit(1);
});
