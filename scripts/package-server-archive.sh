...
# standalone может уже содержать частичный prisma/ — иначе cp создаст prisma/prisma
rm -rf "$STAGE/prisma"
...
cp -a "$ROOT/scripts/prisma-migrate-deploy.cjs" "$STAGE/scripts/"
cp -a "$ROOT/scripts/ensure-tenant-columns-sqlite.cjs" "$STAGE/scripts/"
cp -a "$ROOT/scripts/ensure-user-telegram-phone-sqlite.cjs" "$STAGE/scripts/"
cp -a "$ROOT/scripts/ensure-doctor-extra-columns-sqlite.cjs" "$STAGE/scripts/"
cp -a "$ROOT/scripts/ensure-clinic-price-overrides-sqlite.cjs" "$STAGE/scripts/"
cp -a "$ROOT/scripts/ensure-role-module-access-sqlite.cjs" "$STAGE/scripts/"
cp -a "$ROOT/scripts/split-copy-pricing-from-clients.cjs" "$STAGE/scripts/"
cp -a "$ROOT/scripts/split-copy-orders-from-clients.cjs" "$STAGE/scripts/"
cp -a "$ROOT/scripts/prisma-migrate-deploy.cjs" "$STAGE/prisma-migrate-deploy.cjs"
