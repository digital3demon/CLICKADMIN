/**
 * Перед `migrate deploy`: снимает P3009 для одной известной поломки —
 * миграция 20260512120000_order_kaiten_blocked_at началась (есть строка в
 * `_prisma_migrations`), но `finished_at` / `rolled_back_at` пусты, а колонки
 * в БД ещё нет (или наоборот — колонка есть, а миграция не помечена).
 *
 * Отключить: PRISMA_SKIP_STUCK_MIGRATION_FIX=1
 * Только SQLite / не postgres — скрипт ничего не делает (exit 0).
 */
const { spawnSync } = require("node:child_process");
const fs = require("node:fs");
const path = require("node:path");

const MIGRATION = "20260512120000_order_kaiten_blocked_at";
...
(async () => {
  if (String(process.env.PRISMA_SKIP_STUCK_MIGRATION_FIX || "").trim() === "1") {
    console.log(
      "[prisma-resolve-stuck] PRISMA_SKIP_STUCK_MIGRATION_FIX=1 — пропуск.",
    );
    process.exit(0);
  }
  ...
  try {
    ...
    console.warn(
      "[prisma-resolve-stuck] нет @prisma/client — пропуск авто-исправления:",
  ...
  try {
      ...
      `SELECT
        ...
        EXISTS (
          ...
          WHERE c.table_schema = current_schema()
            ...
            AND c.column_name = 'kaitenBlockedAt'
        ) AS has_col`,
    );
    ...
    console.warn(
      "[prisma-resolve-stuck] запрос к БД не выполнен — пропуск:",
      e instanceof Error ? e.message : e,
    );
  ...
  console.warn(
    `[prisma-resolve-stuck] обнаружена незавершённая миграция ${MIGRATION} (hasCol=${hasCol}) — вызываем migrate resolve.`,
  );
  ...
  if (code !== 0) {
    console.error(
      `[prisma-resolve-stuck] migrate resolve ${flag} завершился с кодом ${code}`,
    );
    process.exit(code);
  }
  console.warn(`[prisma-resolve-stuck] migrate resolve ${flag}: OK`);
  process.exit(0);
})().catch((e) => {
  console.error("[prisma-resolve-stuck]", e);
  process.exit(1);
});
