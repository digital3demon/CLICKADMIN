/**
 * Перед `migrate deploy`: снимает P3009 для 20260825183000_order_invoice_issued_at.
 * Причина — SQLite-тип DATETIME в исходном SQL на PostgreSQL.
 *
...
const MIGRATION = "20260825183000_order_invoice_issued_at";
...
(async () => {
  if (String(process.env.PRISMA_SKIP_STUCK_MIGRATION_FIX || "").trim() === "1") {
    console.log(
      "[prisma-resolve-stuck-invoice-issued-at] PRISMA_SKIP_STUCK_MIGRATION_FIX=1 — пропуск.",
    );
    process.exit(0);
  }
  ...
  try {
    ...
    console.warn(
      "[prisma-resolve-stuck-invoice-issued-at] нет @prisma/client — пропуск:",
  ...
  try {
      ...
      `SELECT
        ...
        EXISTS (
          ...
          WHERE c.table_schema = current_schema()
            ...
            AND c.column_name = 'invoiceIssuedAt'
        ) AS has_col`,
    );
    ...
    console.warn(
      "[prisma-resolve-stuck-invoice-issued-at] запрос к БД не выполнен — пропуск:",
      e instanceof Error ? e.message : e,
    );
  ...
  console.warn(
    `[prisma-resolve-stuck-invoice-issued-at] незавершённая миграция ${MIGRATION} (hasCol=${hasCol}) — migrate resolve.`,
  );
  ...
  if (code !== 0) {
    console.error(
      `[prisma-resolve-stuck-invoice-issued-at] migrate resolve ${flag} завершился с кодом ${code}`,
    );
    await prisma.$disconnect().catch(() => {});
    process.exit(code);
  }
  console.warn(
    `[prisma-resolve-stuck-invoice-issued-at] migrate resolve ${flag}: OK`,
  );

  await prisma.$disconnect().catch(() => {});
  process.exit(0);
})().catch((e) => {
  console.error("[prisma-resolve-stuck-invoice-issued-at]", e);
  process.exit(1);
});
