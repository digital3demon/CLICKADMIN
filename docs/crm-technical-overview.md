...
### ИИ-Админ (Shadow Mode)

Модуль для контроля и обучения ИИ (OpenRouter) на реальных данных лаборатории.

- **Opt-in:** ИИ по умолчанию выключен. Включается и настраивается (API-ключ OpenRouter, выбор моделей) через UI модуля `/ai-admin` (доступно только владельцу). Ключ хранится в БД (`Tenant.openRouterApiKey`), а не в `.env`.
- **Shadow Prediction:** При создании наряда из письма CRM в фоне отправляет текст письма в OpenRouter и сохраняет предсказание (JSON) в `AiOrderPrediction`. Это не блокирует работу администраторов.
- **Diff Viewer:** Владелец может визуально сравнивать, что сохранил администратор и что предложил ИИ.
- **Backtesting & Export:** Скрипты для ретроспективного прогона ИИ по старым письмам и выгрузки датасета (JSONL) для будущего fine-tuning.
