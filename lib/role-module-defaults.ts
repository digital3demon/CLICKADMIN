...
/** Роли, которым по умолчанию включены общие уведомления по нарядам. */
const DEFAULT_ORDERS_NOTIFICATIONS_ROLES: readonly UserRole[] = [
  "ADMINISTRATOR",
  "SENIOR_ADMINISTRATOR",
  "MANAGER",
  "ACCOUNTANT",
...
/** Atomic-модули канбана (кроме базового KANBAN) — для обратной совместимости API. */
export function isKanbanCardSubmodule(module: AppModule): boolean {
  return module.startsWith("KANBAN_");
}

export type AppModuleMatrixGroup = {
  id: string;
  title: string;
  description?: string;
  modules: AppModule[];
...
/** @deprecated Используйте BUNDLE_MATRIX_GROUPS. Оставлено для совместимости импортов. */
export const APP_MODULE_MATRIX_GROUPS: AppModuleMatrixGroup[] = [];

/** Группы пакетов в матрице. */
export { BUNDLE_MATRIX_GROUPS };

/** Порядок пакетов в матрице. */
export function bundlesForAccessMatrix(): BundleId[] {
  return ALL_BUNDLE_IDS;
}

/** Все модули в одном списке (для API и сидов). */
...
export function modulesForAccessMatrix(): AppModule[] {
  return ALL_APP_MODULES.filter((m) => m !== "CLIENTS");
}
...
export const APP_MODULE_LABELS: Record<AppModule, string> = {
  ...
  KANBAN_ATTACH_FILES: "Канбан: прикреплять файлы",
  KANBAN_STOP: "Канбан: СТОП",
  KANBAN_MANAGE_BLOCK: "Канбан: блокировать карточку",
  KANBAN_DELETE_CARD: "Канбан: удалять карточку",
...
export function isClickMigOwnerOnlyBundle(bundle: BundleId): boolean {
  return (
    bundle === "CLICKMIG" ||
    bundle === "CLICKMIG_REVIEW" ||
    bundle === "CLICKMIG_KANBAN" ||
    bundle === "CONFIG_CLICKMIG" ||
    bundle === "AI_ADMIN"
  );
}
...
const KANBAN_WORK_MODULES: readonly AppModule[] = [
  "KANBAN",
  "KANBAN_MOVE_COLUMNS",
  "KANBAN_CARD_CHAT",
  "KANBAN_MANAGE_CHECKLIST",
...
export function defaultModuleAllowed(
  ...
  if (role === "USER") {
    if (KANBAN_WORK_MODULES.includes(module)) return true;
  ...
  if (role === "PRODUCTION" || role === "SENIOR_PRODUCTION") {
    return (
      module === "KANBAN" ||
      module === "KANBAN_MOVE_COLUMNS" ||
      module === "KANBAN_CARD_CHAT" ||
  ...
  if (role === "MANAGER") {
    ...
    return defaultModuleAllowed("ADMINISTRATOR", module);
  }
  ...
  switch (module) {
    ...
    case "KANBAN":
      return true;
    case "KANBAN_MOVE_COLUMNS":
    case "KANBAN_CARD_CHAT":
    ...
    case "KANBAN_ATTACH_FILES":
    case "KANBAN_STOP":
    case "KANBAN_MANAGE_BLOCK":
