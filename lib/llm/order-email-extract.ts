...
export const OrderEmailExtractSchema = z.object({
  ...
  clientOrderText: z
    .string()
    .nullable()
    .describe("Текст заказа клиента дословно, со ссылками на сканы/фото/диск; без подписей и без «Письмо:/От:/Дата:»"),
...
- clientOrderText: дословный текст заказа клиента (конструкции, зубы, цвет, пожелания, ссылки на сканы/фото/Яндекс.Диск — с подписью строки). Без подписей, без «Письмо:/От:/Дата:/Fwd:»
...
- ПММА/PMMA: временная коронка из каталога прайса (не «индивидуализация десны на РММА», не «немедленная нагрузка» — нагрузку только если явно «немедленная нагрузка»). Вариант «… на винтовой фиксации» из прайса — только при скан-маркерах и/или титановых основаниях, либо явной «винтовой фиксации»; иначе обычная временная коронка из прайса. Не выдумывай название вне каталога. Лаб-стенография: «12-22» = мост (зубы 12,11,21,22), «12-22, 24» = мост + отдельный зуб 24 — для коронок quantity по числу единиц (мосты+одиночки), не по числу зубов; основания — quantity = число имплантатов/зубов. Упомянутые основания (бренд из текста → позиция прайса) — отдельная строка. Бренд в «… скан-маркер» ≠ титановое основание того же бренда.
- ПОЖЕЛАНИЯ К КАППЕ: «обрезка не заходя десну», «заканчивалась на зубах» — требования к краю каппы, НЕ отдельная работа «индивидуализация десны». Слово «десну» в контексте обрезки ≠ позиция прайса.
