...
export async function fetchOrdersShipmentListPage(
  ...
  let startIdx = 0;
  if (dec) {
    const found = sorted.findIndex((r) => r.id === dec.i);
    startIdx = found >= 0 ? found + 1 : 0;
  }

  const slice = sorted.slice(startIdx, startIdx + opts.pageSize + 1);
  const hasMore = slice.length > opts.pageSize;
  const pageRaw = hasMore ? slice.slice(0, opts.pageSize) : slice;
  const last = pageRaw[pageRaw.length - 1];

  let orders = await hydrateContractors(pageRaw.map((o) => toOrderListPageRow(o)));
  ...
  return { orders, nextCursor, truncated };
}
